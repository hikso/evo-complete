...
namespace EVO_BusinessLogic
{
    ...
    public class BLArticulo
    {
        ...
        /// <summary>
        /// Integración de artículos , bodegas y artículos de las bodegas de SAP en EVO (artículos creados o editados un día atrás)
        /// </summary>           
        /// <response>bool</response>
        public async Task<bool> AsignarIntegracionArticulosBodegaSAP()
        {
            logger.Info($"Entró al método AsignarIntegracionArticulosBodegaSAP en EVO-WebApi - BLPedidos sin parámetros");

            try
            {
                List<ArticuloSAP> articulosSAP = await ObtenerIntegracionArticulosSAP();

                BOArticulo boArticulo = null;

                foreach (var articuloSAP in articulosSAP)
                {
                    boArticulo = new BOArticulo()
                    {
                    ...
                    if (ObtenerArticuloxCodigoNoExcepcion(articuloSAP.ItemCode) == null)
                    {
                        boArticulo.Nuevo = true;
                    }

                    await AsignarArticulo(boArticulo);
                }

                BLBodega bLBodega = new BLBodega();

                List<BodegaSAP> bodegasSAP = await bLBodega.ObtenerIntegracionBodegasSAP();

                foreach (var bodegaSAP in bodegasSAP)
                {
                    BOBodega bOBodega = new BOBodega()
                    {
                    ...
                    if (bLBodega.ObtenerBodegaxCodigo(bodegaSAP.WhsCode) == null)
                    {
                        bOBodega.Nuevo = true;
                    }                  

                    await bLBodega.AsignarBodega(bOBodega);
                }

                List<ArticuloBodegaSAP> articulosBodegasSAP = await ObtenerIntegracionArticulosBodegasSAP();

                foreach (var articuloBodegaSAP in articulosBodegasSAP)
                {
                    ArticuloBodega boArticuloBodega = new ArticuloBodega()
                    {
                        CodigoArticulo = articuloBodegaSAP.ArticuloSAP.ItemCode,
                        WhsCode = articuloBodegaSAP.BodegaSAP.WhsCode,
                        Stock = string.IsNullOrEmpty(articuloBodegaSAP.OnHand) ? 0 : decimal.Parse(articuloBodegaSAP.OnHand),
                        Minimo = string.IsNullOrEmpty(articuloBodegaSAP.MinStock) ? 0 : decimal.Parse(articuloBodegaSAP.MinStock),
                        Maximo = string.IsNullOrEmpty(articuloBodegaSAP.MaxStock) ? 0 : decimal.Parse(articuloBodegaSAP.MaxStock),
                        Nuevo = false
                    };

                    if (ObtenerArticuloxCodigoBodegaCodigoArticuloNoException(boArticuloBodega.WhsCode,boArticuloBodega.CodigoArticulo) == null)
                    {
                        boArticuloBodega.Nuevo = true;
                    }

                    await AsignarArticuloBodega(boArticuloBodega);
                }
        ...
        public async Task<bool> AsignarArticulo(BOArticulo bOArticulo)
        {
            if (bOArticulo == null)
            {
                EVOException e = new EVOException(errores.errArticuloNoInformado);
            ...
            logger.Info($"Entró al método AsignarArticulo en EVO-WebApi - BLPedidos con parámetro bOArticulo = {JsonConvert.SerializeObject(bOArticulo)}");

            if (string.IsNullOrEmpty(bOArticulo.ItemCode))
            {
            ...
            if (string.IsNullOrEmpty(bOArticulo.ItemName))
            {
                EVOException e = new EVOException(errores.errNombreArticuloNoInformado);
            ...
            if (string.IsNullOrEmpty(bOArticulo.SalUnitMsr))
            {
                EVOException e = new EVOException(errores.errUnidadMedidaNoInformado);
            ...
            try
            {
                await dAArticulo.AsignarArticulo(bOArticulo);
            }
            catch (Exception e)
            {
                throw e;
        ...
        /// <response>bool</response>
        public async Task<bool> AsignarArticuloBodega(ArticuloBodega articuloBodega)
        {
            ...
            logger.Info($"Entró al método AsignarArticuloBodega en EVO-WebApi - BLPedidos con parámetro articuloBodega = {JsonConvert.SerializeObject(articuloBodega)}");
            ...
            try
            {
                await dAArticulo.AsignarArticuloBodega(articuloBodega);
            }
            catch (Exception e)
            {
                throw e;
        ...
        /// <summary>
        /// Integración de artículos de SAP (artículos creados o editados un día atrás)
        /// </summary>      
        /// <response>List<ArticuloSAP></response>
        public async Task<List<ArticuloSAP>> ObtenerIntegracionArticulosSAP()
        {
            logger.Info($"Entró al método ObtenerIntegracionArticulosSAP en EVO-WebApi - BLPedidos sin parámetros");

            ArticuloProxy articuloProxy = new ArticuloProxy();

            List<ArticuloSAP> articulosSAP = null;

            try
            {
                articulosSAP = await articuloProxy.ObtenerIntegracionArticulosSAP();

                logger.Info($"Salió del método ObtenerIntegracionArticulosSAP en EVO-WebApi - BLPedidos con respuesta articulosSAP = {JsonConvert.SerializeObject(articulosSAP)}");
        ...
        {
            logger.Info($"Entró al método ObtenerIntegracionArticulosBodegasSAP en EVO-WebApi - BLPedidos sin parámetros");
            ...
            try
            {
                articulosBodegasSAP = await articuloProxy.ObtenerIntegracionArticulosBodegasSAP();

                logger.Info($"Salió del método ObtenerIntegracionArticulosBodegasSAP en EVO-WebApi - BLPedidos con respuesta articulosBodegasSAP = {JsonConvert.SerializeObject(articulosBodegasSAP)}");
        ...
        /// <summary>
        /// Obtiene artículo por código pero no valida la EVO excepción de artículo no existe. 
        /// </summary>
        /// <param name="codigo">Indica el código del artículo</param>
        /// <returns>Articulo</returns>
        public BOArticulo ObtenerArticuloxCodigoNoExcepcion(string codigo)
        {
            logger.Info($"Entró al método ObtenerArticuloxCodigoNoExcepcion en BLArticulos con parámetro codigo = {codigo}");
        ...
        public List<ArticuloBodega> ObtenerTodosArticulosxBodega(int desde, int hasta, string whsCodePuntoVenta, string whsCodePlanta, string tipoSolicitud)
        {
            ...
            try
            {
                ...
                foreach (ArticuloBodega articulo in articulosBodega)
                {
                    ...
                    {
                        articulo.PedidoSugerido = (articulo.Maximo == null || articulo.Minimo == null)
                            ? 0 : articulo.Maximo - articulo.Minimo;
                    }
                }
        ...
        /// <summary>
        /// Este método obtiene los artículos por el código de la bodega y por el código del artículo sin la excepción de no existe
        /// </summary>
        /// <param name="codigoBodega">Indica el valor del parametro del código de la bodega</param>
        /// <param name="codigoArticulo">Indica el valor del parametro del artículo</param>
        /// <returns>El artículo por el código de la bodega y el código del artículo</returns>
        public ArticuloBodega ObtenerArticuloxCodigoBodegaCodigoArticuloNoException(string codigoBodega, string codigoArticulo)
        {
            if (string.IsNullOrEmpty(codigoBodega))
            {
                EVOException e = new EVOException(errores.codigoBodegaNoInformado);
