...
namespace EVO_BusinessLogic
{
    public class BLBodega
    {
        ...
        public async Task<bool> AsignarBodega(BOBodega bOBodega)
        {
            if (bOBodega == null)
            {
                EVOException e = new EVOException(errores.errBodegaNoInformada);
            ...
            logger.Info($"Entró al método AsignarBodega en EVO-WebApi - BLPedidos con parámetro bOBodega = {JsonConvert.SerializeObject(bOBodega)}");

            if (string.IsNullOrEmpty(bOBodega.WhsCode))
            {
                EVOException e = new EVOException(errores.errCodigoBodegaNoInformada);
            ...
            {
                EVOException e = new EVOException(errores.errNombreBodegaNoInformada);
            ...
            try
            {
                await dABodega.AsignarBodega(bOBodega);
            }
            catch (Exception e)
            {
                throw e;
        ...
        /// </summary>
        /// <param name="codigo">Indica el valor del código del parametro</param>
        /// <returns>BOBodega</returns>
        public BOBodega ObtenerBodegaxCodigo(string codigo)
        {
            if (string.IsNullOrEmpty(codigo))
            {
                EVOException e = new EVOException(errores.errCodigoNoInformado);
            ...
            logger.Info($"Entró al método ObtenerBodegaxCodigo con el parámetro: código : {codigo}");
        ...
        /// <summary>
        /// Integración de bodegas de SAP (creados o editadas un día atrás)
        /// </summary>      
        /// <response>bool</response>
        public async Task<List<BodegaSAP>> ObtenerIntegracionBodegasSAP()
        {
            logger.Info($"Entró al método ObtenerIntegracionBodegasSAP en EVO-WebApi - BLBodega sin parámetros");

            BodegaProxy bodegaProxy = new BodegaProxy();

            List<BodegaSAP> bodegasSAP = null;

            try
            {

                bodegasSAP = await bodegaProxy.ObtenerIntegracionBodegasSAP();

                logger.Info($"Salió del método ObtenerIntegracionBodegasSAP en EVO-WebApi - BLBodega con respuesta bodegasSAP = {JsonConvert.SerializeObject(bodegasSAP)}");
