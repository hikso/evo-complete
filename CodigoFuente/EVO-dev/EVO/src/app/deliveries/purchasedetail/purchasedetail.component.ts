...
export class PurchaseDetailComponent implements OnInit {
    ...
    open(content) {
        ...
        this.isAnyWithoutStock = Object.values<getArticulos>(this.selectedItemsToManage)
        .some(x => (x.stockAlmacenBackUp <= 0 && x.cantidadGestionar > 0));
        
        this.isAnyWithStock = Object.values<getArticulos>(this.selectedItemsToManage)
        .some(x => (x.stockAlmacenBackUp > 0 && x.cantidadGestionar > 0));
        ...
        this.addWithoutStockItems = this.selectedItemsToManage.filter(x => x.stockAlmacenBackUp <= 0 && x.cantidadGestionar > 0);
        this.addWithStockItems = this.selectedItemsToManage.filter(x => x.stockAlmacenBackUp > 0 && x.cantidadGestionar > 0);
