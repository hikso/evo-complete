...
BEGIN
  SET NOCOUNT ON;
  SELECT COUNT(1) AS nRegistros
  FROM Pedidos p WHERE p.WhsCode=@whsCode
END
";
...
BEGIN TRY
  ...
  INSERT INTO @tblNuevoDetallePedidos (DetallePedidoId, ItemCode, EstadoArticuloId, Cantidad, Observacion)
  SELECT DetallePedidoId, ItemCode, EstadoArticuloId, Cantidad , Observacion
  ...
  INSERT INTO DetallePedidos (PedidoId, ItemCode, EstadoArticuloId, Cantidad,Observacion)
  SELECT @PedidoId, tblNuevoDetallePedidos.ItemCode, tblNuevoDetallePedidos.EstadoArticuloId, tblNuevoDetallePedidos.Cantidad,tblNuevoDetallePedidos.Observacion
...
BEGIN
  ...
  FROM (
		...
		INNER JOIN EstadosPedido ep on p.EstadoPedidoId=ep.EstadoPedidoId
		INNER JOIN Bodegas b on b.WhsCode=p.WhsCode
        WHERE ep.Nombre in ('Aceptado','Aceptado parcial')
       ) PaginatedTable
  WHERE (PaginatedTable.RowNum BETWEEN @desde AND @hasta)
  ORDER BY PaginatedTable.FE ASC;
END
	   ...
	   (
	      ...
	      FROM
	         (
				...
				INNER JOIN EstadosPedido ep on p.EstadoPedidoId=ep.EstadoPedidoId 
				INNER JOIN Bodegas b on b.WhsCode=p.WhsCode
				WHERE ep.Nombre IN ('Aceptado parcial','Aceptado')
	        ) Registros 
	        WHERE 
			    ((Registros.Zona LIKE '%' + @Zona + '%') OR (@Zona IS NULL))
            AND ((Registros.Estado LIKE '%' + @Estado + '%') OR (@Estado IS NULL))
...
BEGIN 
		...
		DATEDIFF(DAY,p.FechaPedido,p.FechaEntrega) DiasEntrega
        FROM Pedidos p
		INNER JOIN EstadosPedido ep on p.EstadoPedidoId=ep.EstadoPedidoId 
