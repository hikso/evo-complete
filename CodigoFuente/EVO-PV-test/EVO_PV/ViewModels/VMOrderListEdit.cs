    ...
    public class VMOrderListEdit : NotifyPropertyChanged, INotificationVM, IConfirmationModal
    {
        ...
        private void AddArticlesToOrder()
        {
            ...
            foreach (BOArticle bOArticle in this.ArticlesSelected)
            {
                ...
                bOArticleOrder.Quantity = decimal.Parse(bOArticle.SuggestedOrder);
        ...
        private void SaveOrderEdit(EnumStatesOrder enumStatesOrder, int id)
        {
            ...
            string success = this.OrderListServices.SetOrderEditRequest(bOOrderEditRequest);
            if (success != "false")
            {
                this.notification.Show(DictMessages.Success, enumStatesOrder == EnumStatesOrder.Borrador ? String.Format(DictMessages.RegistrarSolicitudPedidoBorrador, success) : String.Format(DictMessages.RegistrarSolicitudPedidoDefinitivoActualizar, success), NotificationType.Success);
        ...
        private void SaveOrderDuplicate(EnumStatesOrder enumStatesOrder)
        {
            ...
            string success = this.OrderListServices.SetOrderRequest(bOOrderRequest);
            if (success != "false")
            {
                this.notification.Show(DictMessages.Success, enumStatesOrder == EnumStatesOrder.Borrador ? DictMessages.RegistrarSolicitudPedidoBorrador : String.Format(DictMessages.RegistrarSolicitudPedidoDefinitivo, success), NotificationType.Success);
                this.PrincipalScreen.ContentPage.Content = new UCOrderList(this.PrincipalScreen);
            }
            else
            {
