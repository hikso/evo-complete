    ...
    public class VMOrderRequest : NotifyPropertyChanged, INotificationVM, IConfirmationModal
    {
        ...
        private void SetFindArticle()
        {
            if (this.FindArticle != null)
            {
                ...
                this.SuggestedOrderArticleSearch = this.FindArticle.SuggestedOrder;
                this.QuantityArticleSearch = decimal.Parse(this.findArticle.SuggestedOrder != null ? this.FindArticle.SuggestedOrder : "0");
        ...
        private void SaveOrder(EnumStatesOrder enumStatesOrder)
        {
            ...
            string orderRequest = this.orderListService.SetOrderRequest(bOOrderRequest);

            if (orderRequest != "false")
            {
                this.notification.Show(DictMessages.Success, enumStatesOrder == EnumStatesOrder.Borrador ? DictMessages.RegistrarSolicitudPedidoBorrador : String.Format(DictMessages.RegistrarSolicitudPedidoDefinitivo, orderRequest), NotificationType.Success);
