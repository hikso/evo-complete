    ...
    class BoxSettingService : Mapper
    {
        ...
        public async Task<BOBoxState> GetBoxState()
        {
            try
            {
                ...
                using (WebClient client = new WebClient())
                {
                    ...
                    Uri url = new Uri(ConfigurationManager.AppSettings["API_EVO"] + $"caja/estadocaja?codigoPuntoVenta=PV-PRA");
        ...
        public async Task<BOBoxSetting> GetBoxSettingHeader()
        {
            try
            {
                ...
                {
                    CredentialCache cc = new CredentialCache();
                    Uri url = new Uri(ConfigurationManager.AppSettings["API_EVO"] + $"caja/apertura?codigoPuntoVenta=PV-PRA");
                    client.UseDefaultCredentials = true;
                    client.Encoding = Encoding.UTF8;
                    var HtmlResult = await client.DownloadStringTaskAsync(url.AbsoluteUri);
                    ...
                    deliveries.CodePointOfStore = "PV-PRA";
                }
