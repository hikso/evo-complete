    ...
    public class BillingService : Mapper
    {
        ...
        public async Task<List<BOBillingArticle>> GetArticles(string codigoPuntoVenta, string customerCode)
        {
            ...
            try
            {
                ...
                {
                    Uri url = new Uri($"{ConfigurationManager.AppSettings["API_EVO"]}articulos/facturacion?identificacionSocio={customerCode}&codigoPuntoVenta={codigoPuntoVenta}");
        ...
        public async Task<List<BOPayWays>> GetPaymentWays()
        {
            ...
            try
            {
                using (WebClient wc = new WebClient())
                {
                    Uri url = new Uri($"{ConfigurationManager.AppSettings["API_EVO"]}facturacion/formaspago");
                    wc.UseDefaultCredentials = true;
                    wc.Encoding = Encoding.UTF8;
                    var HtmlResult = await wc.DownloadStringTaskAsync(url.AbsoluteUri);
        ...
        public string GenerateInvoice(BOGenerateInvoice bOGenerateInvoice)
        {
            try
            {
                Uri url = new Uri(ConfigurationManager.AppSettings["API_EVO"] + "facturacion");
        ...
        public async Task<List<BOBank>> GetBanks()
        {
            ...
            try
            {
                using (WebClient wc = new WebClient())
                {
                    Uri url = new Uri($"{ConfigurationManager.AppSettings["API_EVO"]}facturacion/bancos");
                    wc.UseDefaultCredentials = true;
                    wc.Encoding = Encoding.UTF8;
                    var HtmlResult = await wc.DownloadStringTaskAsync(url.AbsoluteUri);
        ...
        public async Task<BOBillingArticle> GetArticle(string customerCode, string code, string name)
        {
            ...
            try
            {
                using (WebClient wc = new WebClient())
                {
                    Uri url = new Uri($"{ConfigurationManager.AppSettings["API_EVO"]}articulo/puntoventa/filtrar?codigoCliente={customerCode}&codigo={code}&nombre={name}");
                    wc.UseDefaultCredentials = true;
                    wc.Encoding = Encoding.UTF8;
                    var HtmlResult = await wc.DownloadStringTaskAsync(url.AbsoluteUri);
