    ...
    class ArticleService : Mapper
    {
        ...
        public async Task<BOPaginationArticle> GetArticlesByWhsCodeSale(int from, int to, string whsCodeFactory, int typeOrderId)
        {
            try
            {
                ...
                using (WebClient client = new WebClient())
                {
                    ...
                    Uri url = new Uri(domain + $"articulos/bodega?desde={from}&hasta={to}&whsCodePuntoVenta={ConfigurationManager.AppSettings["CODIGO_PUNTO_VENTA"]}&whsCodePlanta={whsCodeFactory}&tipoSolicitud={typeOrderId}");

                    //Uri url = new Uri(ConfigurationManager.AppSettings["API_EVO_PV"] + $"articulos/bodega?desde={from}&hasta={to}&whsCodePuntoVenta={ConfigurationManager.AppSettings["CODIGO_PUNTO_VENTA"]}&whsCodePlanta={whsCodeFactory}&tipoSolicitud={typeOrderId}");
        ...
        public async Task<List<BOActions>> GetActions()
        {
            ...
            try
            {
                ...
                using (WebClient client = new WebClient())
                {
                    ...
                    Uri url = new Uri(domain + "articulos/accion");
        ...
        public async Task<List<BOPackage>> GetPackages()
        {
            ...
            try
            {
                ...
                using (WebClient client = new WebClient())
                { //ConfigurationManager.AppSettings["API_EVO"]
                    
                    string domain = appConfiguration.AppSettings["API_EVO"].ToString();
                    Uri url = new Uri(domain + "articulos/empaques");
        ...
        public async Task<List<BOStateArticle>> GetStatesArticles()
        {
            ...
            try
            {
                ...
                using (WebClient client = new WebClient())
                {
                    ...
                    Uri url = new Uri(domain + "articulos/obtenerTodosEstados");
        ...
        public List<BOArticle> GetArticlesSearch(BOSearchArticleRequest bOSearchArticleRequest)
        {
            ...
            {
                string domain = appConfiguration.AppSettings["API_EVO"].ToString();
                Uri url = new Uri(domain + "articulos/bodega/buscar");
        ...
        public async Task<List<BOArticleReceive>> GetArticlesToBeReceive(string DeliveryId)
        {
            ...
            {
                string domain = appConfiguration.AppSettings["API_EVO"].ToString();
                Uri url = new Uri(domain + $"articulos/recepcion?id={DeliveryId}");
