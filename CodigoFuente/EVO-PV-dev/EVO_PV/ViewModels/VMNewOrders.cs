...
namespace EVO_PV.ViewModels
{
    ...
    public class VMNewOrders : NotifyPropertyChanged
    {
        ...
        public bool EnableNewOrder
        {
            get { return enableNewOrder; }
            set
            {
                enableNewOrder = value;
                this.OnPropertyChanged("EnableNewOrder");
            }
        }
        ...
        public int OrderTypeSelectedId
        {
            ...
            set
            {
                ...
                this.EnableNewOrder = true;
                if (OrderTypeSelected.OrderTypeName==EnumTypeRequest.Compras.ToString())
                {
                    BOGetOrderErasedRequest request = new BOGetOrderErasedRequest()
                    {
                        ...
                        CodeFactory = ConfigurationManager.AppSettings[EnumConstanst.CODIGO_PUNTO_VENTA.ToString()]
                    };                    

                    if (this.OrderListServices.GetStateErasedOrderRequestFactory(request))
                    {
                        this.EnableNewOrder = false;
