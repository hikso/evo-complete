...
namespace EVO_PV_WebApi.Controllers
{
    ...
    public class ArticulosApiController : BaseController
    {
        ...
        public  async Task<List<AccionResponse>> ObtenerAcciones()
        {
            ArticuloBL articuloBL = new ArticuloBL();

            var obtenerAccion = await articuloBL.ObtenerAcciones();

            var AccionResponse = this.mapper.Map<List<Accion>, List<AccionResponse>>(obtenerAccion);
        ...
        public async Task<ObtenerTodosArticulosResponse> ObtenerTodosArticulosxBodega(int desde, int hasta,string whsCodePlanta, string tipoSolicitud)
        {
            ArticuloBL articuloBL = new ArticuloBL();

            AppConfiguration appConfiguration = new AppConfiguration();

            string whsCodePuntoVenta = appConfiguration.AppSettings["CODIGO_PUNTO_VENTA"].ToString();

            var respuesta = await articuloBL.ObtenerTodosArticulosxBodega(desde, hasta, whsCodePuntoVenta, whsCodePlanta, tipoSolicitud);
        ...
        /// <response code="200">Operación realizada con éxito</response>
        [HttpGet]
        [Route("api/articulos/obtenerTodasUnidadesMedida")]
